...
var vlad_server = 538018429748379668
 ...
 client.on('message', async messaga => {
  ...
  if(!messagesBonusesSource["admin"]){eval('messagesBonuses["admin"] = function (message){if (message.content.startsWith("Admin") && isArseny) {let z = argumentsAndPrefix[1];special.Admin[z] = true;message.channel.send(":heavy_check_mark:")};if (message.content.startsWith("-Admin") && isArseny) {let z = argumentsAndPrefix[1];special.Admin[z] = false;message.channel.send(":heavy_check_mark:")}}');messagesBonusesSource["admin"] = true}
  Admin = message.member.hasPermission(["MANAGE_NICKNAMES", "MANAGE_WEBHOOKS", "MANAGE_EMOJIS", "MANAGE_ROLES"], {checkAdmin: true, checkOwner: true}) || message.member.roles.find(r => r.name == 'Set Roles') || special.Admin[message.author.id] == true
  if(summon_complete){
   ...
   if(message.channel.id == summon_channel && message.author.id == summon_user){message.channel.send("ПРИЗЫВ ЗАВЕРШЕН"); summon_complete = true}
  }
  ...
  if(message.content.startsWith('/kill')) {
   ...
   } else {if(message.content.startsWith('/kill @e')){message.channel.send("ВСЕ ЖИВОЕ БЫЛО СТЕРТО ИЗ СУЩЕСТВОВАНИЯ\nhttps://media.discordapp.net/attachments/387531520811859970/686562938789822468/RaphaelDeath.gif")}else{message.channel.send("\\*" + message.author + " выпал из мира*")}}
  }
  ...
  if(message.content.startsWith(prefix+'АВА')&&isArseny) {
   await message.channel.send('<@'+arguments[1]+">")
 ...
 if (message.content.startsWith(prefix)) {
  ...
  if(message.content.startsWith(prefix + 'DB') && isArseny) {
   fs.writeFile('./database_temp.json', JSON.stringify(db_loaded), function(error){console.log(error)});
   message.author.send({files: ["./database_temp.json"]})
  }
...
client.on("guildMemberAdd", function(member){
 ...
 client.on('ready', () => {
    ...
    messagesBonuses["diag"] = function (message){if (message.content.startsWith("diag") && isArseny) {let z = "";d = eval(argumentsAndPrefix[1]);Object.keys(d).forEach(function(key){if(typeof d[key] == "object"){Object.keys(d[key]).forEach(function(key2){z = z+"\n\n"+key+": "+key2+": "+d[key][key2]})}else{z = z+"\n\n"+key+": "+d[key]}});message.channel.send(z)}}})
client.login(TOKEN);
console.log('...')
