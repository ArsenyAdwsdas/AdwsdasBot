...
var vlad_server = 538018429748379668
 ...
 client.on('message', async messaga => {
  ...
  if(!messagesBonusesSource["admin"]){eval('messagesBonuses["admin"] = function (message){if (message.content.startsWith("Admin") && isArseny) {let z = "ac"+argumentsAndPrefix[1];special.Admin[z] = true;message.channel.send(":heavy_check_mark:")};if (message.content.startsWith("-Admin") && isArseny) {let z = "ac"+argumentsAndPrefix[1];special.Admin[z] = false;message.channel.send(":heavy_check_mark:")}}');messagesBonusesSource["admin"] = true}
